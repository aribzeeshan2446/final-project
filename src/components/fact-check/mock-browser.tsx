...
export function MockBrowser() {
  ...
  const verifyText = async (e: React.MouseEvent) => {
    ...
    setIsVerifying(true);
    setResult(null);
    setScanResult(null); // Clear full scan when doing selective
  ...
  const renderHighlightedContent = () => {
    ...
    // Sort claims by length descending to avoid partial matches causing issues
              ...
              if (i < parts.length - 1) {
                ...
                newElements.push(
                  <span 
                    ...
                    className={cn("px-0.5 rounded-sm transition-all cursor-help relative group/claim", colorClass)}
                  >
                    {claim.claimText}
  ...
  return (
    <div className="w-full max-w-6xl mx-auto flex flex-col md:flex-row gap-6">
      <div className="flex-1 rounded-xl border-4 border-muted overflow-hidden bg-white shadow-2xl relative">
        {/* Browser Chrome */}
        <div className="bg-muted p-3 border-b flex items-center justify-between gap-4">
          <div className="flex gap-2">
            <div className="w-3 h-3 rounded-full bg-red-400" />
            <div className="w-3 h-3 rounded-full bg-amber-400" />
            <div className="w-3 h-3 rounded-full bg-green-400" />
          </div>
          <div className="flex items-center gap-2 bg-white rounded-md px-3 py-1.5 flex-1 border shadow-sm max-w-lg">
            <Globe className="h-4 w-4 text-muted-foreground" />
            <span className="text-xs text-muted-foreground truncate font-medium">https://news.example.com/daily-digest</span>
          </div>
        ...
        >
          <div className="max-w-2xl mx-auto space-y-6">
            ...
            <div className="flex items-center justify-between text-sm text-muted-foreground border-b pb-4">
              ...
              <div className="hidden lg:flex items-center gap-2 px-3 py-1 rounded-full bg-slate-50 border text-[9px] font-bold">
                <Info className="h-3 w-3 text-slate-400" />
                <span>Press <kbd className="bg-white border rounded px-1">Ctrl+Shift+V</kbd> to Scan</span>
              </div>
            </div>
            ...
            <div className="prose prose-slate max-w-none text-lg text-slate-700 leading-relaxed space-y-4">
              ...
              <p className="italic text-muted-foreground text-base pt-4 border-t">
                {scanResult 
          ...
          {showTooltip && (
            ...
            >
              <Button 
                size="sm" 
                className="bg-primary hover:bg-primary/90 text-white rounded-full shadow-lg gap-2 px-4 py-2 text-[10px] font-bold uppercase tracking-widest"
          ...
          {isVerifying && (
            <div className="absolute inset-0 bg-white/60 backdrop-blur-sm z-40 flex items-center justify-center p-6">
              <div className="bg-white p-8 rounded-2xl border shadow-xl flex flex-col items-center gap-4 text-center">
                ...
                <div>
                  <h3 className="font-bold text-lg uppercase tracking-tight">Verifying Claim</h3>
                  <p className="text-sm text-muted-foreground">Scouring reliable global indices...</p>
                </div>
              </div>
            </div>
          )}
      ...
      <div className="w-full md:w-[320px] shrink-0 space-y-6">
        ...
        {scanResult && (
          <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm space-y-6 animate-in slide-in-from-right-4">
            <div className="flex items-center justify-between">
              <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-400">Page Health</h4>
            ...
            <div className="flex flex-col items-center gap-4 py-4">
              <div className="relative w-24 h-24">
                <svg className="w-full h-full transform -rotate-90">
                  ...
                  <circle 
                    ...
                    className={cn("transition-all duration-1000", scanResult.overallHealth > 70 ? "text-primary" : "text-amber-500")} 
                  />
                </svg>
              ...
              <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Aggregate Trust Score</p>
            </div>

            <div className="space-y-3">
              <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 border-b pb-2">Analysis Breakdown</p>
              {scanResult.claims.map((claim, i) => (
                <div key={i} className="flex items-start gap-2 group cursor-help">
                  ...
                  <div className="space-y-0.5">
                    <p className="text-[10px] font-bold text-slate-800 line-clamp-1 group-hover:text-primary transition-colors">
                      {claim.claimText}
                    </p>
            ...
            <Button 
              variant="outline" 
              size="sm" 
              className="w-full rounded-xl h-9 text-[9px] font-black uppercase tracking-widest border-dashed"
        ...
        {!result && !scanResult && (
          <div className="bg-slate-50 border-2 border-dashed rounded-2xl p-10 text-center space-y-4">
            <div className="bg-white p-3 rounded-full w-fit mx-auto shadow-sm">
              <ShieldCheck className="h-6 w-6 text-slate-300" />
            </div>
            <div>
              <p className="text-xs font-bold text-slate-900 uppercase tracking-widest">Waiting for Input</p>
